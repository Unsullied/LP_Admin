...
export default function PersonaVersionPage() {
  ...
  const baseBoundariesText = useMemo(
    () => (version?.hardBoundaries ?? []).join('\n'),
  ...
  const saveDraft = useCallback(
    async (mode: 'auto' | 'manual') => {
      ...
      if (current.boundariesInput !== baseBoundariesText)
        patch.hardBoundaries = parseLines(current.boundariesInput)
      ...
      <div className={ui.topbar}>
        <div style={{ display: 'flex', gap: 10, alignItems: 'center', flexWrap: 'wrap' }}>
          ...
          <div className={ui.muted}>
            {version ? `v${version.version} · ${version.personaId}` : ''}
          </div>
        </div>
